...
public class DingUser extends Model<DingUser> {
    ...
    @ApiModelProperty(value = "是否号码隐藏：
            ...
            @ApiModelProperty(value = "扩展属性，可以设置多种属性，在使用该参数前，需要先在OA管理后台增加该属性，然后再调用接口进行赋值。详情请参见关于extension参数的使用。手机上最多只能显示10个扩展属性。该字段的值支持链接类型填写，同时链接支持变量通配符自动替换，目前支持通配符有：userid，corpid。示例： [工位地址](http://www.dingtalk.com?userid=#userid#&corpid=#corpid#)。")
