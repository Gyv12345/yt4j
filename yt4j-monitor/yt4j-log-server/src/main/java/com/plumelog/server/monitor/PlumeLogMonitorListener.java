...
/**
 * className：PlumeLogMonitorListener description： 日志监控报警 time：2020-07-02.11:19
 *
 * @author Tank
 * @version 1.0.0
 */
...
public class PlumeLogMonitorListener implements ApplicationListener<PlumelogMonitorEvent> {
	...
	public void parserLogMessage(List<RunLogMessage> logMessages) {
		logMessages.forEach(runLogMessage -> {
			List<WarningRule> monitorRuleConfig = plumeLogMonitorRuleConfig
					.getMonitorRuleConfig(runLogMessage.getAppName());
	...
	private void earlyWarning(WarningRule rule, long count, String key, String errorContent, String className) {
		PlumeLogMonitorTextMessage plumeLogMonitorTextMessage = new PlumeLogMonitorTextMessage.Builder(
				rule.getAppName()).className(rule.getClassName()).errorCount(rule.getErrorCount()).time(rule.getTime())
						.count(count).monitorUrl(getMonitorMessageURL(rule, className)).errorContent(errorContent)
						.build();
	...
	private void sendMesageES(WarningRule rule, long count, String errorContent) {
		try {
			...
			elasticLowerClient.insertListComm(Arrays.asList(object.toJSONString()),
					LogMessageConstant.PLUMELOG_MONITOR_MESSAGE_KEY, LogMessageConstant.ES_TYPE);
	...
	private String getMonitorMessageURL(WarningRule rule, String className) {
		...
		builder.append(url).append("/#/?appName=").append(rule.getAppName()).append("&className=").append(className)
				.append("&logLevel=ERROR").append("&time=").append(startTime).append(",").append(currentTime);
