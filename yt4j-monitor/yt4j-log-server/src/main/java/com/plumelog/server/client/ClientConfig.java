...
/**
 * className：RedisClientConfig description： TODO time：2020-07-02.15:51
 *
 * @author Tank
 * @version 1.0.0
 */
...
public class ClientConfig implements InitializingBean {
	...
	public RedisClient initRedisClient() {
		if (StringUtils.isEmpty(redisHost)) {
			logger.error(
					"can not find redisHost config! please check the application.properties(plumelog.redis.redisHost) ");
			return null;
		}
	...
	public RedisClient initRedisQueueClient() {
		if (StringUtils.isEmpty(queueRedisHost)) {
			logger.error(
					"can not find redisHost config! please check the application.properties(plumelog.queue.redis.redisHost) ");
			return null;
		}
		...
		if (hs.length == 2) {
			...
			logger.error(
					"redis config error! please check the application.properties(plumelog.queue.redis.redisHost) ");
			return null;
		}
	...
	public KafkaConsumer initKafkaConsumer() {
		if (InitConfig.KAFKA_MODE_NAME.equals(model)) {
			if (StringUtils.isEmpty(kafkaHosts)) {
				logger.error(
						"can not find kafkaHosts config! please check the application.properties(plumelog.kafka.kafkaHosts) ");
				return null;
			}
			return KafkaConsumerClient.getInstance(kafkaHosts, InitConfig.KAFKA_GROUP_NAME, InitConfig.MAX_SEND_SIZE)
					.getKafkaConsumer();
		}
		else {
			return null;
