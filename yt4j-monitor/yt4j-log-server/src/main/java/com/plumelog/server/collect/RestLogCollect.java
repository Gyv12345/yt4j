...
/**
 * className：RedisLogCollect description：RedisLogCollect 获取rest接口中日志，存储到es
 *
 * @author Frank.chen
 * @version 1.0.0
 */
public class RestLogCollect extends BaseLogCollect {
	...
	private void collectRuningLog() {
		while (true) {
			...
			try {
				logs = PlumeRestClient.getLogs(this.restUrl + "?maxSendSize=" + InitConfig.MAX_SEND_SIZE + "&logKey="
						+ LogMessageConstant.LOG_KEY, this.restUserName, this.restPassWord);
	...
	private void collectTraceLog() {
		while (true) {
			...
			try {
				logs = PlumeRestClient.getLogs(this.restUrl + "?maxSendSize=" + InitConfig.MAX_SEND_SIZE + "&logKey="
						+ LogMessageConstant.LOG_KEY_TRACE, this.restUserName, this.restPassWord);
				if (logger.isDebugEnabled()) {
					logs.forEach(log -> {
						logger.debug(log);
					});
