...
import { list, del } from '@/api/sys/dictItem'
...
export default {
  ...
  data () {
    return {
      ...
      loadData: (parameter) => {
        return list(Object.assign(parameter, this.queryParam, { code: this.code })).then((res) => {
          return res.result
        })
      },
      selectedRowKeys: [],
  ...
  methods: {
    ...
    statusFilter (status) {
      const statusValue = this.statusDict.filter((item) => item.code === status)
